...
class ZebPayClient {
    constructor(config) {
        this.cachedUsdRate = null;
        this.rateLastFetched = null;
        this.config = config;
        ...
        this.exchangeRateClient = axios_1.default.create({
    ...
    async fetchTokenPrice(symbol) {
        ...
        for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
            try {
                ...
                const tokenData = response.data.find((item) => item.virtualCurrency === searchSymbol && item.currency === 'INR');
                if (!tokenData) {
    ...
    async validateWithToken(symbol) {
        try {
    ...
    categorizeValidationError(error, token, attempt) {
        const baseContext = {
            attempt,
            timestamp: new Date(),
            endpoint: '/market'
    ...
    createEnhancedApiError(type, message, statusCode, token, retryable, context) {
